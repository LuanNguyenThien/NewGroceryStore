...
public class PanelLoading extends javax.swing.JLayeredPane {
    ...
    public void doneLogin(NhanVien nv_data) {
        isMessage = false;
        this.nv_data = nv_data;
        ImageIcon icon = Util.byteArrayToImageIcon(nv_data.getHinhAnh());
        pic.setIcon(icon);
        cmdContinue.setText("Continue with " + nv_data.getHoTen());
        animator.start();
    }
    ...
    private void cmdEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdEditActionPerformed
        ...
        if (option == JFileChooser.APPROVE_OPTION) {
            ...
            byte[] blobimage = Util.imageIconToByteArray(icon);
            pic.setIcon(icon);
            repaint();
            try {
                String sql = "update `nhanvien` set `HinhAnh`=? where MaNV=? limit 1";
                PreparedStatement p = DatabaseConnection.getInstance().getConnection().prepareStatement(sql);
                p.setBytes(1, blobimage);
                p.setString(2, "NV0001");
                p.execute();
                nv_data.setHinhAnh(blobimage);
